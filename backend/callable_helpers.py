...
All Cloud Function handlers receive a Flask / functions-framework Request
object and return a (body_dict, status_code, headers_dict) tuple.

Auth: in production the Firebase ID token is sent in ``data.idToken`` inside
the request body, because the API Gateway replaces the ``Authorization``
header with its own service-account JWT when proxying to Cloud Run. In local
development (no gateway) the token is still read from the
``Authorization: Bearer`` header as a fallback.
"""
...
def verify_firebase_token(request: flask.Request) -> dict[str, Any]:
    ...
    Token source priority:
      1. Request body ``data.idToken`` — used in production via API Gateway.
      2. ``Authorization: Bearer <token>`` header — used in local dev.
